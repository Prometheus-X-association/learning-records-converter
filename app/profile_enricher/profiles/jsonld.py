...
from pydantic import BaseModel, Field, AnyUrl, field_validator, model_validator, ValidationInfo
...
class Concept(ProfileElement):
    ...
    @staticmethod
    def related_only_for_deprecated(value: Optional[list[AnyUrl]], info: ValidationInfo) -> Optional[list[AnyUrl]]:
        """
        Validator to ensure 'related' field is used only for deprecated concepts.
        """
        if value and not info.data.get('deprecated'):
            raise ValueError("'related' MUST only be used on Concepts that are deprecated")
        return value
...
class Extension(ProfileElement):
    ...
    iriSchema: Optional[AnyUrl] = Field(default=None, alias="schema")  # Rename internal Pydantic field
    inlineSchema: Optional[dict] = None

    @field_validator('recommendedActivityTypes')
    @staticmethod
    def validate_recommended_activity_types(value: Optional[list[AnyUrl]], info: ValidationInfo) -> Optional[list[AnyUrl]]:
        """
    ...
    @staticmethod
    def validate_recommended_verbs(value: Optional[list[AnyUrl]], info: ValidationInfo) -> Optional[list[AnyUrl]]:
        """
    ...
    def validate_schema_fields(self) -> 'Extension':
        ...
        if self.iriSchema is not None and self.inlineSchema is not None:
            raise ValueError("Profiles MUST use at most one of schema and inlineSchema for Extensions")
        return self
...
class DocumentResource(ProfileElement):
    ...
    def validate_schema_fields(self) -> 'DocumentResource':
        ...
        """
        if self.iriSchema is not None and self.inlineSchema is not None:
            raise ValueError("Profiles MUST use at most one of schema and inlineSchema for Document Resources")
        return self
...
class StatementTemplateRule(BaseModel):
    ...
    any: Optional[list[str | dict]] = None
    all: Optional[list[str | dict]] = None
    none: Optional[list[str | dict]] = None
    ...
    def validate_jsonpath(value: Optional[str]) -> Optional[str]:
        ...
        if re.search(r'\(.*\)', value):
            raise ValueError("Filter and script expressions MUST NOT be used in JSONPath")
        return value
    ...
    def at_least_one_rule(self) -> 'StatementTemplateRule':
        ...
        if not any([self.presence, self.any, self.all, self.none]):
            raise ValueError("A Statement Template Rule MUST include one or more of presence, any, all, or none")
        return self
...
class StatementTemplate(ProfileElement):
    ...
    def not_both_object_ref_and_activity_type(self) -> 'StatementTemplate':
        ...
        """
        if self.objectStatementRefTemplate is not None and self.objectActivityType is not None:
            raise ValueError("A Statement Template MUST NOT have both objectStatementRefTemplate and objectActivityType")
        return self
...
class Pattern(ProfileElement):
    ...
    @staticmethod
    def primary_must_have_label_and_definition(value: Optional[LanguageMap], info: ValidationInfo) -> Optional[LanguageMap]:
        """
    ...
    def validate_alternates(value: Optional[list[AnyUrl]]) -> Optional[list[AnyUrl]]:
        ...
        if value is not None:
            if any(x.endsWith('/optional') or x.endsWith('/zeroOrMore') for x in value):
                raise ValueError("MUST NOT put optional or zeroOrMore directly inside alternates")
        return value

    @field_validator('sequence')
    @staticmethod
    def validate_sequence(value: Optional[list[AnyUrl]], info: ValidationInfo) -> Optional[list[AnyUrl]]:
        """
        Validator to ensure that sequences include at least two members, unless specific conditions apply.
        """
        if value is not None:
            if len(value) < 2 and not (info.data.get('primary') and not info.data.get('inScheme')):
                raise ValueError("MUST include at least two members in sequence, unless sequence is in a primary Pattern that is not used elsewhere and the member of sequence is a single Statement Template")
        return value
    ...
    def exactly_one_pattern_type(self) -> 'Pattern':
        ...
        """
        pattern_types = ['alternates', 'optional', 'oneOrMore', 'sequence', 'zeroOrMore']
        if sum(1 for pt in pattern_types if getattr(self, pt) is not None) != 1:
            raise ValueError("A Pattern MUST contain exactly one of alternates, optional, oneOrMore, sequence, and zeroOrMore")
        return self
    ...
    def no_self_reference(self) -> 'Pattern':
        ...
        def check_self_reference(pattern_id, pattern_list):
            if pattern_id in pattern_list:
                raise ValueError("MUST NOT include any Pattern within itself, or within any Pattern within itself, or at any depth")

        for field in ['alternates', 'optional', 'oneOrMore', 'sequence', 'zeroOrMore']:
            value = getattr(self, field)
            if value:
                check_self_reference(self.id, value if isinstance(value, list) else [value])
        return self
...
class Profile(BaseModel):
    ...
    """
    id: AnyUrl
    context: Annotated[AnyUrl, Literal[PROFILE_CONTEXT_URL]] = Field(default=PROFILE_CONTEXT_URL)
    type: ProfileTypeEnum
    conformsTo: Annotated[AnyUrl, Literal[PROFILE_CONFORMS_TO_URL]] = Field(default=PROFILE_CONFORMS_TO_URL)
    prefLabel: LanguageMap
    definition: LanguageMap
    ...
    @staticmethod
    def validate_see_also(value: Optional[AnyUrl], info: ValidationInfo) -> Optional[AnyUrl]:
        """
        ...
        if value and len(info.data.get('definition', {}).get('en', '')) > 100:
            raise ValueError("When seeAlso is provided, definition SHOULD only include a short description of the Profile")
        return value
    ...
    def unique_pattern_ids(self) -> 'Profile':
        ...
        for pattern in self.patterns or []:
            if pattern.id in pattern_ids or pattern.id == self.id:
                raise ValueError(f"Pattern id {pattern.id} is not unique or is the same as the Profile id")
            ...
            if version.wasRevisionOf:
                for revision in version.wasRevisionOf:
                    if revision not in all_ids:
                        raise ValueError(f"wasRevisionOf {revision} does not refer to an existing version")
        return self
    ...
    def validate_concept_references(self) -> 'Profile':
        ...
        for concept in self.concepts or []:
            if isinstance(concept, Concept):
                for field in ['broader', 'broadMatch', 'narrower', 'narrowMatch', 'related', 'relatedMatch', 'exactMatch']:
                    refs = getattr(concept, field, None)
                    if refs:
                        for ref in refs:
                            if ref not in concept_ids:
                                raise ValueError(f"Concept {concept.id} references non-existent concept {ref} in {field}")
        return self
